...
import { 
  Search, Sparkles, Heart, MessageSquare, Share2, Copy, Download, 
  HelpCircle, ChevronRight, Check, CheckCircle2, RefreshCw, AlertCircle
} from 'lucide-react';
...
export default function HomePage() {
  ...
  return (
    <>
      ...
      <main className="flex-grow hero-gradient pb-20">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            {['success', 'leadership', 'productivity', 'faith', 'discipline'].map((cat) => {
              ...
              return (
                <div 
                  key={cat}
                  className="glass-card p-5 text-left flex flex-col justify-between hover:scale-[1.02] duration-300 relative overflow-hidden group min-h-[190px]"
                >
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-12 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-3">
            <button
              ...
              className={`px-4 py-2 rounded-xl text-xs font-semibold tracking-wide transition-all border flex items-center gap-1.5 cursor-pointer ${
                selectedMood === null
                          ...
                          {user ? (
                            <form onSubmit={(e) => handlePostComment(e, quote.id)} className="flex gap-2">
                              <input
                                ...
                                className="flex-grow bg-slate-50 dark:bg-slate-900/50 text-xs px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-800 outline-none focus:border-purple-500"
                              />
                              <button
                                type="submit"
